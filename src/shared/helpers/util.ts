...
export const hexInputValidation = (hex: string): boolean => {
...
export const sanitizeHex = (hexcolor: string) => {
  if (!hexcolor) return ''
  if (hexcolor.length === 4) {
    const str = hexcolor.split('#')?.[1]
    return (
      '#' +
      str
        .split('')
...
export async function asyncSome(array: any, callback: any) {
...
export const clearLocalStorage = () => {
  const layerMappingStr: any = localStorage.getItem('CONTRACT_LAYERS_MAP')
  const termsStr = localStorage.getItem(`terms`) || '{}'
  const termsObj: any = JSON.parse(termsStr)
  localStorage.clear()
  localStorage.setItem('CONTRACT_LAYERS_MAP', layerMappingStr)
